<template>
  <v-row :id="id" v-bind="getRow" v-resize.quiet="onResize">
    <!-- FORM-BASE TOP SLOT -->
      ...
      >
        ...
        <slot :name="getTypeItemSlot(obj)" v-bind="{ obj, index, id }">
          ...
          <slot :name="getKeyItemSlot(obj)" v-bind="{ obj, index, id }">
            ...
            <!-- ARRAY -->
            <template v-if="obj.schema.type === 'array'">
              <div
            ...
            >
              <v-icon v-if="obj.schema.iconLeft" left :dark="obj.schema.dark">
                {{ obj.schema.iconLeft }}
              </v-icon>
              {{ setValue(obj) }}
              ...
              {{ obj.schema.label }}
              <v-icon v-if="obj.schema.iconRight" right :dark="obj.schema.dark">
    ...
    methods: {
      ...
      mapTypeToComponent(type) {
        ...
        const allTypeComponents = {
          ...typeToComponent,
          // ...Vue.options.components, // TODO: check this later
        };
        ...
        return allTypeComponents[type] ? allTypeComponents[type] : type;
      },
      ...
      getPropertyClassName(obj) {
        return this.getPropertyClassNameWithAppendix(
          obj,
          propertyClassAppendix,
        );
      },
      ...
      getInjectedScopedSlots(id, obj) {
        ...
        const rx = new RegExp(
          `${injectSlotAppendix}-(.*?)-${keyClassAppendix}`,
        );
      ...
      onIntersect(entries, observer, obj) {
        const isIntersecting = entries.length
      ...
      // EMIT EVENT
      emitValue(event, val) {
        let emitEvent = change.includes(event)
          ? "change"
      ...
      deprecateEventCustomID(ev) {
        console.warn(
          `--- DEPRECATION ${ev}:${this.id}: ----------------------------------------------------------------------------`,
        );
        console.warn(
          `<form-base  @${ev}:${this.id}="handler" /> is deprecated use simplified version <form-base  @${ev}="handler" />`,
        );
        console.warn(
          `---------------------------------------------------------------------------------------------`,
        );
      ...
      deprecateCombinedEvents(emitEvent, event) {
        if (emitEvent !== event) {
          console.warn(
            `--- DEPRECATION Combined Listener:  --------------------------------------------------------------------------`,
          );
          console.warn(
            `Combined Event-Listener '${emitEvent}' have been removed for better comprehensibility and simplification`,
          );
          console.warn(
            `Please use separate listener for each event like <form-base  @focus="handler" @input="handler" @blur="handler"/>`,
          );
          console.warn(
            `---------------------------------------------------------------------------------------------`,
          );
      ...
      setObjectByPath(object, path, value) {
        ...
        pathArray.forEach((p, ix) => {
          // if (ix === pathArray.length - 1) this.$set(object, p, value); // TODO:
      ...
      autogenerateSchema(value) {
        ...
        schema = JSON.parse(schema, (key, val) => {
          ...
          if (typeof val === "boolean")
            return defaultSchemaIfValueIsBoolean(key);
          return val;
        });
      ...
      rebuildArrays(model, schema) {
        ...
        if (!model)
          throw `Property 'model' is null or undefined. Use '<form-base :model="myModel" :schema="mySchema" />'. myModel must be at least an empty Object.`;

        // noise if async loading
        if (isEmpty(schema) && isEmpty(model)) {
          console.warn(
            `At least one of the properties 'model' or 'schema' in <form-base /> must be at least an empty Object. Ignore this Warning on async loading 'model' or 'schema' `,
          );
          // return
        }
